...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        printf(
            "%zu anagrammes trouvés en %zu μs pour le mot %s avec le tableau "
            "de mots !\n\n",
        ...
        word_dict_search_anagrams(dictionnaire, buf, result_dict);
        gettimeofday(&end_dict, NULL);

        size_t elapse_dict = (end_dict.tv_sec - start_dict.tv_sec) +
                             (end_dict.tv_usec - start_dict.tv_usec);

        // On affiche le résultat de la table de hachage
        word_array_print(result_dict);
        printf("%zu anagrammes trouvés en %zu μs pour le mot %s avec la "
               "table de hâchage !\n\n",
